...
1. **Build tool: tsdown** (pin exact 0.22.x), single config `tsdown.config.ts` with two entries — `src/cli.ts` (CLI, shebang) and `src/lib.ts` (library) — ESM output to `dist/`, `dts: true`, sourcemaps, `clean: true`. Keep `tsc --noEmit` as the separate typecheck gate (agent2linear pattern, TS_EXISTING_REPO_REVIEW.md:61). Enable `isolatedDeclarations` in tsconfig so dts uses the fast oxc path (couples to T-tsconfig; stated as a dependency, not decided here).
...
- Couplings (stated, not decided here): package manager (tie-break #1) — tsdown is PM-agnostic; tsconfig topic must set `isolatedDeclarations` for the fast dts path and keep the strict-ESM baseline; release topic owns release-please/publish workflow details; docs topic decides whether any sdist-style docs shipping matters (npm answer: it doesn't — docs stay in the repo/site, not the tarball).
...
Proposed — orchestrator gates acceptance. Key open dependency: none blocking within T02; couplings to T-runtime (Node floor), T-tsconfig (`isolatedDeclarations`), T-release (release-please/publish workflow ownership) flagged above.
...
| Color (picocolors + semantic helpers) | Replace Python-specific tool with TypeScript equivalent (rich color → picocolors) |
...
*Color:* **picocolors 1.1.1** — zero deps, ~7 kB, CJS+ESM, `NO_COLOR`-friendly, `createColors(enabled)` for manual gating (https://github.com/alexeyraspopov/picocolors); **chalk 5.6.2** — ESM-only, larger, equally fine but org precedent is anti-chalk (claim-npm review line 320).
...
2. **Color: picocolors** + a small semantic helper module (error=red, notice=yellow, success=green) built with `createColors(enabled)`.
...
7. **`--no-color`: implement for real** — flag + `NO_COLOR` env + `FORCE_COLOR` override + TTY detection wired into the picocolors `createColors` gate (and passed to cli-table3/spinner so they render plain).
...
- UX libs: the source's rich/questionary/pyperclip trio has no single TS equivalent, so intent is preserved per concern with the smallest maintained, current libraries: picocolors (org anti-chalk precedent + NO_COLOR), yocto-spinner (stderr default matches R7.1 — progress is diagnostics), @inquirer/checkbox (only candidate with documented `output: process.stderr` + name/value choices + a first-party testing package), cli-table3 (de-facto standard; only dep string-width), clipboardy (only serious cross-platform clipboard lib in Node).
...
- **yocto-spinner over ora**: gives up ora's richer API and battle-testing to avoid a chalk+picocolors double color stack; both are Sindre Sorhus-maintained, so maintenance risk is comparable.
...
- Runtime deps added: `commander`, `picocolors`, `cli-table3`, `yocto-spinner`, `@inquirer/prompts`, `clipboardy` (6 deps replacing click/rich/questionary/pyperclip). All ESM-compatible; clipboardy and yocto-spinner are ESM-only — consistent with the strict-ESM default (Synthesis item 3).
...
2. **Node matrix**: two-entry LTS matrix = `[engines floor, Active LTS]` — concretely `["22", "24"]` if the runtime topic sets the floor at 22; if it adopts the org's newest floor of Node ≥24 (contributors-please/`-action`), collapse to `["24"]` and add `26` when it becomes LTS (Oct 2026). Never include EOL Node 20.
...
- Matrix floor+LTS: preserves the source's "minimum + one newer" intent with currently-supported Node lines; Node 20 is EOL so the index's example `["20","22"]` guess must not be used.
...
Proposed — orchestrator gates acceptance. Flagged for explicit orchestrator attention: (a) pinning tie-break #7 resolution (hybrid policy), (b) osv-scanner vs PM-native audit in the manual scan, (c) omission of changelog.yml, (d) matrix values pending the runtime-floor topic.
...
- M. FUNDING.yml — **Reuse existing repo decision** (copy as-is)
...
- **CLA (H)**: (1) hosted **cla-assistant.io** — free SAP-hosted service, still operating; main repo's latest release v2.13.1 (2023-08), aging but functional (https://github.com/cla-assistant/cla-assistant). (2) **cla-assistant/github-action** ("lite") — repo pushed 2026-03-23, latest tag v2.6.1; self-contained Actions-based signature flow (https://github.com/cla-assistant/github-action via GitHub API, checked 2026-07-07).
...
- **H. CLA — keep the full program as-is**: CLA texts, FAQ, setup guide, CONTRIBUTING.md front-loading, cla-assistant.io as the documented default (zero-infra, matches source); add one paragraph to the setup guide noting the actively-maintained `cla-assistant/github-action` lite variant as a self-contained alternative for template adopters who prefer Actions-only.
...
- **cla-assistant hosted**: SAP's hosted service shows slow maintenance (last release 2023); if it sunsets, adopters switch to the documented github-action variant — hence the added paragraph rather than a migration now.
