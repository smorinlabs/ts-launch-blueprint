...
- **Features/capabilities**: Install via `uv pip install pytest`; conventions (`tests/` dir, `test_*.py` files, `test_` functions); plain-assert examples; run via `uvx pytest`; feature list (assertions, discovery, fixtures, parametrization, plugins); `pyproject.toml` config example (`[tool.pytest.ini_options]` with `testpaths`, `python_files`); fixture example using `tmp_path`; best practices (isolation, descriptive names, edge cases, Arrange-Act-Assert); troubleshooting (discovery naming, editable install for imports). Note a doc/config discrepancy: the doc says test files may start with `test_` or end with `_test.py` (pytest.md:18), while the repo config narrows discovery to `test_*.py` only (`python_files = ["test_*.py"]`, pyproject.toml:128) — the ported doc should match the actual config.
...
- **Python-specific assumptions**: `subprocess`, `re`. The actual invoker is `.github/workflows/update-contributors.yml` line 49 (`python scripts/update_contributors.py`); the Justfile `update-contributors` recipe (Justfile:303-312) does NOT call the script — it reimplements the job with `git shortlog -sne`, overwriting CONTRIBUTORS.md wholesale and destroying the marker region the script preserves. This workflow-vs-recipe divergence is a source inconsistency the port should reconcile (make both paths call the one script).
...
**Behavior**: Triggers on push/PR to main + dispatch, declares `contents: write`, checks out with `fetch-depth: 0` (full history, as changelog generation needs) — and then has no further steps. It generates nothing. (Actual changelog tooling in the repo is `cog.toml` / cocogitto, used locally.)
...
**Behavior**: `py-projects --version` prints the package version and exits 0. The version string is never hand-maintained: it is derived from git tags at build time (setuptools-scm/hatch-vcs), written into a generated `_version.py` which is lint-excluded (`pyproject.toml:114`) but — despite its own header comment saying "don't track in version control" — IS git-tracked at the pinned SHA (a source inconsistency; the versioning-release area notes the same), with a `fallback_version` for tag-less builds and `no-local-version` so builds from dirty trees don't get `+g<sha>` suffixes. The CLI imports from the generated file; the package `__init__` independently resolves the same version from installed metadata. The test asserts the printed version matches `__version__` (case-insensitively).
...
**TS representation**: `--version` flag reading `package.json`'s `version` (via `createRequire`/JSON import or a build-time injected constant). The git-tag-derived versioning half is a release-tooling decision (semantic-release, changesets, or manual tags — needs research, belongs to the packaging/release feature area); the TS port should deliberately gitignore its generated version artifact rather than copy the source's tracked-generated-file inconsistency. The CLI-area contract to preserve is: `--version` exits 0, prints exactly the packaged version, and a test asserts CLI output == package.json version.
...
**Intent**: Scriptability — callers can branch on the exit code to distinguish "fix your config" from "the API is down" from "file a bug"; empty results are deliberately not failures; tracebacks are opt-in diagnostics, never shown to normal users. Note that the documented exit-code table (`EXAMPLECLI.md:107-115`) does not match the implementation: it lists 2 = "Authentication error" and 5 = "User interrupt" (never emitted by the code) and labels 4 as "Input/Output error" (the code uses 4 for any unexpected exception) — the port must reconcile docs with behavior.
...
### Advertised-but-unimplemented fuzzy search (port must decide, not copy)

**Where**: `py_launch_blueprint/projects.py:24-25` (module docstring: "with support for fuzzy matching and various output formats"); `EXAMPLECLI.md:3` (features list: "Fuzzy search for project names"); `pyproject.toml:32-33` (runtime deps `thefuzz>=0.19.0` + `python-Levenshtein>=0.21.0`, the latter commented "For better performance with thefuzz").

**Behavior**: Fuzzy search is advertised in both the module docstring and the documented feature list, and two runtime dependencies are declared to support it — but `thefuzz` is never imported anywhere in `py_launch_blueprint/` or `tests/`. No fuzzy filtering happens: the only project-name narrowing is questionary's checkbox prompt (plain substring typeahead from the prompt library, not thefuzz). The deps are dead weight and the claim is untrue at the pinned SHA.

**Intent**: The *intended* feature is fuzzy filtering of project names during selection; the implementation never landed (same pattern as the dead `--no-color` flag above).

**TS representation**: The port must make a deliberate decision rather than copy the inconsistency: either (a) implement fuzzy filtering for real (candidates: fuse.js, fuzzysort — needs research, coordinate with the prompt-library choice since some multi-select prompts have built-in filtering), or (b) drop the claim from the docs/docstring AND omit the fuzzy-matching dependencies entirely. Record the choice in TS_PORT_DECISIONS.md; do not ship advertised-but-dead capability or unused runtime deps.
...
**Intent**: Errors should teach the fix — a first-run user gets the complete onboarding recipe in the failure message instead of a bare stack trace. Configuration failures have a stable, documented exit code (1) distinct from API errors (3) and generic errors (4) for scriptability. Doc/code mismatch to reconcile in the port: `EXAMPLECLI.md:108-115` also documents exit codes 2 ("Authentication error") and 5 ("User interrupt") that the code never emits, and labels 4 as "Input/Output error" while the code uses 4 for any unexpected exception.
...
**Behavior**: Bare `just` lists all recipes in file order (unsorted, so the curated ordering is preserved). Every recipe carries one or more `[group(...)]` attributes; the documented taxonomy block (Justfile:55-73) covers SETUP through QUICK START only (setup/install/update/dev/test/build/run/docs/pre-commit/help/utilities/debug/releases/workflow/quick start), with a "quick start" group cross-cutting the essentials (`install-dev`, `check`, `run`, `dev`). Two additional groups — `clean` (Justfile:252) and `legacy` (Justfile:380) — are used by recipes but absent from the documented block, a source drift the port could fix by adding them to the taxonomy comment. High-frequency recipes get single-letter/two-letter aliases. `just select-shell` opens the interactive `--choose` picker. A Windsurf glob rule (`justfile-rules.md`, triggered on `Justfile`) enforces the house style — blank line between recipes, every recipe in an existing group, doc comment directly above each recipe — so AI edits preserve the convention.
...
### Template-rename seam: centralized project-name variables in the Justfile
**Where**: `Justfile` lines 2-4 (project-wide `py_package_name := "py_launch_blueprint"` / `repo_name := "py-launch-blueprint"` / `command_name := "py-projects"` variables)
**Behavior**: The Justfile centralizes the three project identifiers (package name, repo name, CLI command name) as top-of-file variables so recipes never hard-code them — a single edit point when the template is adopted under a new name. This variable trio is the tracked, in-scope template-rename seam at the pinned SHA.
**Intent**: Make renaming a template instance a one-place edit for the whole task-runner surface instead of a grep-and-replace across every recipe.
**TS representation**: Keep the three name variables at the top of the ported Justfile (package name / repo name / CLI bin name) with all recipes referencing them.
**Scope exclusion (D-005)**: `TEMPLATE_USAGE.md` and `scripts/init_from_template.py` / `scripts/cleanup_template.py` / `scripts/rename_template.py` are UNTRACKED at the pinned SHA (4828f85) and are D-005 scope exclusions — they are referenced here for context only and MUST NOT generate port tasks. Any scripted clone→rename→clean→verify instantiation flow for the TS template is a separate future decision, not part of this port.
...
### GitHub-Actions-testing recipe trio: disposable test repo for workflow changes
**Where**: `Justfile` lines 418-619 — `pr-to-testrepo` (line 420), `clean-pr-to-testrepo` (line 500), `create-test-pr` (line 539); see also the Part 1 `Justfile` entry.
**Behavior**: Three `[group('workflow')]` recipes let a maintainer reproduce and test GitHub Actions changes in a disposable private repository via the `gh` CLI: `pr-to-testrepo` checks out a PR's code and creates a new test repo from it (writing a `pr2testrepo.txt` marker file identifying repos created this way), `create-test-pr` creates a branch + test PR to trigger the workflows, and `clean-pr-to-testrepo` deletes the remote repo and local directory — refusing (with a warning) when the marker-file safety check suggests the target was not created by `pr-to-testrepo`. The creation/deletion recipes are gated with just's `[confirm(...)]` prompts, and every step has colored, actionable error handling (gh missing, checkout/pull failures).
**Intent**: Actions workflows can only truly be tested by running them; this gives a safe, repeatable, one-command loop for exercising workflow changes against a throwaway repo without polluting the real one — with marker files and confirm gates making the destructive half hard to misfire.
**TS representation**: Language-agnostic (git + gh + shell) — port the trio nearly verbatim in the TS Justfile, keeping the `[confirm]` gates, the marker-file safety check, and the colored precondition errors. Flagged explicitly so Phase 4/5 planning does not overlook these recipes.
...
**Behavior**
CLI tests never touch the network, the terminal's interactive prompt, the clipboard, or the user's real config. Each boundary is replaced: the API client constructor is patched where `projects.py` imports it, config resolution returns a constructed `Config`, `questionary.checkbox(...).ask()` returns a canned selection list, and `pyperclip.copy` is a spy asserted with `mock_copy.assert_called_with("1")` (test_cli.py:185). `Mock(spec=PyClient)` constrains the mock to the real client's interface so calls to nonexistent methods fail. Caveat: three tests mock the prompt at the wrong seam — test_cli.py:82, 100, 155 set `mock_checkbox.ask.return_value`, but the code calls `questionary.checkbox(...).ask()`, so the canned value never reaches it; the actual return is a truthy `MagicMock` that iterates as empty. Only test_cli.py:121 and 182 use the correct `mock_checkbox.return_value.ask.return_value`. The tests still pass, but the three mis-mocked ones exercise the empty-selection path rather than what they appear to pin.
...
**TS representation**
`vi.mock`/`vi.spyOn` at module boundaries if Vitest is chosen; the `spec=` interface-conformance property maps to typed mocks (e.g., `vi.mocked`, or `vitest-mock-extended` — needs research). The interactive-prompt seam (questionary → likely `@inquirer/prompts` or `prompts` in TS — needs research) and clipboard (`clipboardy` candidate) must remain injectable/mocked the same way. The port should mock the prompt correctly (the equivalent of `return_value.ask.return_value`, not the mis-mocked seam noted above) and decide per test which path — a real selection or the empty-selection no-op — each ported test is meant to pin.
...
- `py_launch_blueprint/projects.py:352-353` (error print + `sys.exit(1)`); `projects.py:401-403` (`PyError`/API error → exit 3), `projects.py:404-408` (any other unexpected exception → exit 4, traceback only under `--verbose`) — the 3/4 paths are currently untested; `projects.py:383-385` (user cancel / empty selection prints "No projects selected" and returns → exit 0)

**Behavior**
Every test asserts the process exit code, making exit status part of the pinned interface: 0 for success (including the "user selected nothing" path — a benign early return with a yellow "No projects selected" notice, test_cli.py:84), 1 for missing token with a specific error message. Source also defines exit 3 (`PyError`, i.e. API errors) and exit 4 (any other unexpected exception, with a traceback only when `--verbose` is set) which the suite does not yet cover — a known coverage gap, not a contract change.
...
**TS representation**
Assert `process.exitCode`/thrown exit-override value on every CLI test. Consider closing the gap by also pinning exit 3 (API error) and exit 4 (unexpected exception) in the port (flag as an improvement, not a behavior change).
...
**Behavior**
The three `--format` modes are validated behaviorally: JSON output must be machine-parseable (`json.loads`) after removing the human-facing `Fetching projects...` progress line — necessary because the progress text really does share stdout with the data: `Progress()` is created with no console argument (`projects.py:358`), so it renders to rich's default stdout console; the stderr-bound `error_console` (`projects.py:50`) is never used for progress. CSV must contain the `id,name` header; text must contain the project id. `--output <file>` must create the file with exactly the formatted content (trailing whitespace stripped in the assertion); `--copy` must pass the formatted string to the clipboard function.

**Intent**
Formats are consumed by other tools, so validity (parseable JSON, CSV header) is tested rather than exact string equality. The regex-strip (`tests/test_cli.py:125`) is a workaround for a known source wart: progress output lands on stdout alongside the data (the stderr `error_console` exists but is not wired to `Progress`), so piped JSON is in fact contaminated in the source.

**TS representation**
Same behavioral assertions: `JSON.parse` the captured stdout, check CSV header, check file contents for `--output`, spy on clipboard for `--copy`. Routing progress to stderr in the TS port is a deliberate improvement over the source (not preservation): once stdout carries only data, the regex-strip becomes unnecessary — capture streams separately and assert stdout is pure data (make the improvement explicit in TS_PORT_DECISIONS.md).
...
- `docs/source/reference/versioning.md` (user-facing description of the versioning scheme)
...
Known inconsistency in the template: `_version.py` says "don't track in version control" but IS git-tracked at the pinned SHA (it is not in `.gitignore`). Treat the generated-and-untracked variant as the intent. The reference doc also has two inaccuracies — source-doc defects NOT to carry into the rewritten TS doc: `versioning.md:10` claims a fallback version of `0.0.0` but `pyproject.toml:73` sets `fallback_version = "0.0.1"`; and `versioning.md:6` claims dev versions include the commit ID by default (`1.2.4.dev1+gabcdef12`) but `local_scheme = "no-local-version"` (`pyproject.toml:176-177`) strips the local `+g<sha>` segment.
