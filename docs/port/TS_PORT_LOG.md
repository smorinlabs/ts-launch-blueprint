...
## 2026-07-10 — Post-port swap: pnpm 10 (D-035) — merged

- **Trigger**: user selection review; verification spike wf_b15b5505-bd8. Executor: Opus; validators: 2× Sonnet (D-034).
- **Change**: packageManager pnpm@10.34.3+sha512 (corepack hex hash — executor caught that npm's dist.integrity base64 is the WRONG artifact and would brick fresh clones; proven by live self-management download); pnpm-lock.yaml imported from package-lock.json (402 resolutions preserved), package-lock deleted; .npmrc strict settings (pnpm 10 doesn't read devEngines — npm entry removed, node runtime entry kept); all 7 workflows on pnpm/action-setup @0ebf4713 (# v6.0.9, SHA verified) before setup-node cache:pnpm with frozen-lockfile; publish.yml installs/builds with pnpm but PUBLISHES with npm (D-035: pnpm 10 OIDC delegation has an unresolved field failure; Node 24's npm ≥11.5.1 is the reliable Trusted Publishing path); Justfile/Makefile/lefthook/docs (~25 files) swapped; pack-check keeps npm pack (pnpm pack --json shape differs); 6 new meta-tests.
